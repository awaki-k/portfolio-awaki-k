...
module mlp #(
    ...
    // Quantization
    // ---------------------------------
    reg  signed [SC_WIDTH-1:0] sc_r;
    reg         [SH_WIDTH-1:0] sh_r;
    reg                        rq_param_v;
    reg                        rq_go_v;   
    wire rq_param_w = sc_rd_valid & sh_rd_valid; // params available in this cycle
    ...
    always @(posedge clk) begin
        if (reset) begin
            ...
            // Stage1: latch params (load pulse)
            rq_param_v <= rq_param_w;
            if (sc_rd_valid) sc_r <= $signed(sc_rd_data);
            if (sh_rd_valid) sh_r <= sh_rd_data;

            // Stage2: start pulse (1-cycle after load)
            rq_go_v <= rq_param_v;
        end
    end
